...
@RestController
@RequestMapping("/api/v1/revocation-requests")
@Tag(name = "Solicitudes de revocación", description = "Inicio de solicitudes y consulta de existencia de credenciales.")
public class RevocationRequestController {

	private final RevocationRequestInitiationService service;
	private final FlowSessionService flowSessions;
	private final FlowSessionCookieService sessionCookies;

	public RevocationRequestController(RevocationRequestInitiationService service,
			FlowSessionService flowSessions, FlowSessionCookieService sessionCookies) {
		this.service = service;
		this.flowSessions = flowSessions;
		this.sessionCookies = sessionCookies;
	}

	@Operation(operationId = "initiateRevocationRequest",
			summary = "Inicia una solicitud y consulta si existen credenciales disponibles",
	...
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Resultado normalizado de existencia de credenciales",
					headers = @Header(name = CorrelationIdFilter.HEADER_NAME, description = "Identificador de correlación",
							schema = @Schema(type = "string")),
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
							schema = @Schema(implementation = RevocationRequestResponse.class))),
			...
			@ApiResponse(responseCode = "409", description = "Conflicto controlado: AVAILABILITY_CHECK_IN_PROGRESS, REVOCATION_REQUEST_IN_PROGRESS o CONCURRENT_REQUEST", headers = @Header(name = CorrelationIdFilter.HEADER_NAME, schema = @Schema(type = "string")), content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ApiError.class))),
	...
	public ResponseEntity<RevocationRequestResponse> initiate(
			@io.swagger.v3.oas.annotations.parameters.RequestBody(required = true,
				description = "DNI y evidencia efímera reCAPTCHA que se validan en el backend antes de crear una solicitud.",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
							schema = @Schema(implementation = StartRevocationRequest.class)))
			@Valid @RequestBody StartRevocationRequest body,
			@Parameter(hidden = true) HttpServletRequest request) {
		String correlationId = String.valueOf(request.getAttribute(CorrelationIdFilter.REQUEST_ATTRIBUTE));
		RevocationRequestResponse result = service.initiate(body.dni(), body.recaptchaToken(), correlationId);
		ResponseEntity.BodyBuilder response = ResponseEntity.ok();
		if (result.canContinue()) {
			response.headers(sessionCookies.headers(flowSessions.establish(result.requestId())));
		}
		else {
			response.headers(sessionCookies.clearHeaders());
		}
