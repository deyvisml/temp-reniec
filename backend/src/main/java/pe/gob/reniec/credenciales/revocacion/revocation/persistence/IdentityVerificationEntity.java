...
public class IdentityVerificationEntity {
	...
	@Column(name = "error_or_revocation_code", length = 64)
	private String errorOrRevocationCode;
	...
	public IdentityVerificationEntity(DigitalCredentialRevocationRequestEntity request, int attemptNumber,
	...
	public void finish(IdentityVerificationStatus status, IdentityMatchResult matchResult,
			Instant completionTime, String externalReference, String errorOrRevocationCode) {
		finish(status, matchResult, completionTime, externalReference, errorOrRevocationCode, null, null, null);
	}

	public void finish(IdentityVerificationStatus status, IdentityMatchResult matchResult,
			Instant completionTime, String externalReference, String errorOrRevocationCode,
			String providerSessionState, String verifiedSubjectHash) {
		finish(status, matchResult, completionTime, externalReference, errorOrRevocationCode,
				providerSessionState, verifiedSubjectHash, null);
	}

	public void finish(IdentityVerificationStatus status, IdentityMatchResult matchResult,
			Instant completionTime, String externalReference, String errorOrRevocationCode,
			String providerSessionState, String verifiedSubjectHash, String verifiedFirstName) {
		...
		if (status == IdentityVerificationStatus.VERIFIED
				&& (verifiedFirstName == null || verifiedFirstName.isBlank())) {
			throw new IllegalArgumentException("verifiedFirstName is required for a verified identity");
		}
		if (status != IdentityVerificationStatus.VERIFIED && verifiedFirstName != null) {
			throw new IllegalArgumentException("verifiedFirstName is only valid for a verified identity");
		}
		...
		this.errorOrRevocationCode = bounded(errorOrRevocationCode, 64);
		...
		this.verifiedFirstName = bounded(verifiedFirstName, 100);
	...
	public String getVerifiedFirstName() { return verifiedFirstName; }
	public IdentityMatchResult getDniMatchResult() { return dniMatchResult; }
	public Instant getStartedAt() { return startedAt; }
	public Instant getCompletedAt() { return completedAt; }
	public String getErrorOrRevocationCode() { return errorOrRevocationCode; }
