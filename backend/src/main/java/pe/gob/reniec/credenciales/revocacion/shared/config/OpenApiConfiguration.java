package pe.gob.reniec.credenciales.revocacion.shared.config;
...
public class OpenApiConfiguration {
	...
	@Bean
	OpenAPI revocationDigitalCredentialsOpenApi() {
		return new OpenAPI().components(new Components().addSecuritySchemes("FlowSessionCookie",
				new SecurityScheme().type(SecurityScheme.Type.APIKEY).in(SecurityScheme.In.COOKIE)
						.name("revocacion_access").description("Access JWT de la sesión transaccional, transportado exclusivamente mediante cookie HttpOnly."))
				.addSecuritySchemes("FlowRefreshCookie", new SecurityScheme().type(SecurityScheme.Type.APIKEY)
						.in(SecurityScheme.In.COOKIE).name("revocacion_refresh")
						.description("Refresh JWT rotatorio, transportado exclusivamente mediante cookie HttpOnly.")))
				.info(new Info()
				.title("API de revocación de credenciales digitales")
				...
				.tags(List.of(
						new Tag().name("Solicitudes de revocación")
								.description("Inicio de solicitudes y consulta de disponibilidad de credenciales."),
						new Tag().name("Verificación de identidad")
								.description("Inicio, callback, estado e invalidación temporal de ID Perú."),
						new Tag().name("Sesión del flujo").description("Consulta, actualización y cierre de la operación ciudadana activa."),
						new Tag().name("Credenciales vigentes").description("Listado autenticado y selección de credenciales de la operación activa."),
