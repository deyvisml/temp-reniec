...
import pe.gob.reniec.credenciales.revocacion.revocation.initiation.RevocationRequestProtectedException;
import pe.gob.reniec.credenciales.revocacion.revocation.initiation.RevocationRequestConcurrencyException;
import pe.gob.reniec.credenciales.revocacion.revocation.initiation.AvailabilityCheckInProgressException;
import pe.gob.reniec.credenciales.revocacion.revocation.initiation.AvailabilityProviderException;
import pe.gob.reniec.credenciales.revocacion.revocation.initiation.AvailabilityTimeoutException;
import pe.gob.reniec.credenciales.revocacion.revocation.initiation.AvailabilityUnavailableException;
...
public final class GlobalExceptionHandler {
	...
	@ExceptionHandler(RevocationRequestConcurrencyException.class)
	ResponseEntity<ApiError> handleRevocationRequestConcurrency(RevocationRequestConcurrencyException exception,
			HttpServletRequest request) {
	...
	@ExceptionHandler(RevocationRequestProtectedException.class)
	ResponseEntity<ApiError> handleProtectedRevocationRequest(RevocationRequestProtectedException exception,
			HttpServletRequest request) {
		return respond(HttpStatus.CONFLICT, "REVOCATION_REQUEST_IN_PROGRESS",
	...
	ResponseEntity<ApiError> handleAvailabilityUnavailable(AvailabilityUnavailableException exception,
		...
		return respond(HttpStatus.SERVICE_UNAVAILABLE, "AVAILABILITY_UNAVAILABLE",
				"No podemos consultar las credenciales en este momento. Inténtalo más tarde.", request);
	}
	...
	ResponseEntity<ApiError> handleSession(FlowSessionException exception, HttpServletRequest request) {
		return switch (exception.reason()) {
			...
			case REFRESH_CONFLICT -> respond(HttpStatus.CONFLICT, "SESSION_REFRESH_IN_PROGRESS", "La sesión ya se está actualizando en otra pestaña.", request);
	...
	@ExceptionHandler(DigitalCredentialListingException.class)
	ResponseEntity<ApiError> handleDigitalCredentialListing(DigitalCredentialListingException exception,
			HttpServletRequest request) {
		return switch (exception.reason()) {
			case IDENTITY_REQUIRED, NOT_ALLOWED -> respond(HttpStatus.FORBIDDEN,
					"DIGITAL_CREDENTIAL_STEP_NOT_ALLOWED", "Verifica tu identidad antes de continuar.", request);
			case IN_PROGRESS -> respond(HttpStatus.CONFLICT, "DIGITAL_CREDENTIAL_LIST_IN_PROGRESS",
					"La consulta ya se está procesando. Inténtalo nuevamente en unos segundos.", request);
			case TIMEOUT -> respond(HttpStatus.GATEWAY_TIMEOUT, "DIGITAL_CREDENTIAL_LIST_TIMEOUT",
					"La consulta tardó demasiado. Inténtalo nuevamente.", request);
			case UNAVAILABLE -> respond(HttpStatus.SERVICE_UNAVAILABLE, "DIGITAL_CREDENTIAL_LIST_UNAVAILABLE",
					"No podemos consultar las credenciales en este momento.", request);
			case INVALID_PROVIDER_RESPONSE -> respond(HttpStatus.UNPROCESSABLE_ENTITY,
					"DIGITAL_CREDENTIAL_LIST_INVALID_RESPONSE",
					"No fue posible interpretar la lista de credenciales. Inténtalo nuevamente.", request);
			case EMPTY -> respond(HttpStatus.UNPROCESSABLE_ENTITY, "DIGITAL_CREDENTIAL_LIST_EMPTY",
					"Actualmente no existen credenciales disponibles para revocar.", request);
			case INVALID_SELECTION -> respond(HttpStatus.UNPROCESSABLE_ENTITY,
					"INVALID_DIGITAL_CREDENTIAL_SELECTION",
					"Selecciona credenciales válidas de esta operación.", request);
			case CONFLICT -> respond(HttpStatus.CONFLICT, "DIGITAL_CREDENTIAL_SELECTION_CONFLICT",
					"La selección cambió simultáneamente. Recarga la lista e inténtalo nuevamente.", request);
		};
	}

	@ExceptionHandler(RevocationConfirmationException.class)
	ResponseEntity<ApiError> handleRevocationConfirmation(RevocationConfirmationException exception,
			HttpServletRequest request) {
		return switch (exception.reason()) {
			case IDENTITY_REQUIRED, NOT_ALLOWED -> respond(HttpStatus.FORBIDDEN,
			...
			case INVALID_REASON -> respond(HttpStatus.UNPROCESSABLE_ENTITY,
					"INVALID_REVOCATION_REASON", "El motivo registrado ya no es válido.", request);
			case INVALID_SELECTION -> respond(HttpStatus.UNPROCESSABLE_ENTITY,
					"INVALID_DIGITAL_CREDENTIAL_SELECTION", "La selección de credenciales ya no es válida.", request);
			...
			case DEPENDENCY_UNAVAILABLE -> respond(HttpStatus.SERVICE_UNAVAILABLE,
					...
					"No podemos revocar la credencial en este momento. Inténtalo más tarde.", request);
		};
	}
