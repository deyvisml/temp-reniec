...
class BackendHttpIntegrationTests {
	...
	void missingCaptchaUsesDedicatedSafeErrorBeforePersistence() throws Exception {
		HttpResponse<String> response = postRevocation("{\"dni\":\"00000001\"}", "captcha-required-test");
	...
	void openApiDocumentsEveryPublicOperationAndSchema() throws Exception {
		...
		assertThat(applicationPaths).containsExactlyInAnyOrder(
				"/api/v1/system/status", "/api/v1/revocation-requests",
		...
		assertThat(document)
				.contains("\"/actuator/health\"", "getSystemStatus", "initiateRevocationRequest",
						"getCurrentRequestDigitalCredentials", "previewCurrentRevocation",
						"getConfirmedRevocationReview", "confirmCurrentRevocation",
						"RevocationReviewRequest", "RevocationReviewResponse",
						"RevocationConfirmationRequest", "DigitalCredentialListResponse",
						"getActuatorHealth", "Solicitudes de revocación", "Estado técnico",
						"StartRevocationRequest", "RevocationRequestResponse", "SystemStatusResponse",
						...
						"RECAPTCHA_INVALID_RESPONSE", "REVOCATION_REQUEST_IN_PROGRESS", "date-time", "[0-9]{8}",
						"\"200\"", "\"400\"", "\"409\"", "\"415\"", "\"500\"",
				...
				.doesNotContain("/__test/", "/actuator/info", "/actuator/env",
						...
						"eligibilityResult", "digitalCredentialCount",
	...
	void openApiYamlIsAvailableForDevelopmentTooling() throws Exception {
		...
		assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
		assertThat(response.body()).contains("openapi:", "/api/v1/revocation-requests:", "/actuator/health:");
	}
	...
	@Test
	void revocationReviewAndConfirmationRequireTheFlowSession() throws Exception {
		HttpResponse<String> review = send(HttpRequest.newBuilder(
				uri("/api/v1/revocation-requests/current/review")).GET().build());
		HttpResponse<String> confirmation = send(HttpRequest.newBuilder(
				uri("/api/v1/revocation-requests/current/confirmation"))
				.header(HttpHeaders.CONTENT_TYPE, "application/json")
		...
		assertThat(confirmation.body()).contains("\"code\":\"SESSION_REQUIRED\"")
				.doesNotContain("REVOCACION_CREDENCIALES_DIGITALES_V1", "trace");
	}
	...
	private HttpResponse<String> postRevocation(String body, String correlationId) throws Exception {
		return send(HttpRequest.newBuilder(uri("/api/v1/revocation-requests"))
				.header(HttpHeaders.CONTENT_TYPE, "application/json")
				.header(CorrelationIdFilter.HEADER_NAME, correlationId)
