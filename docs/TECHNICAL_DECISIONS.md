...
- Después de un resultado inicial positivo, el segundo servicio puede reflejar un cambio externo y devolver una lista vacía o sin vigentes. La fotografía completa se conserva, el avance se bloquea y el resultado se presenta como ausencia actual de credenciales disponibles, no como falla técnica.
...
- La consulta detallada se reserva en una transacción breve, ejecuta la llamada externa fuera de la transacción y reemplaza atómicamente la colección completa de credenciales vigentes y revocadas. Cada entrada, regreso, recarga o reintento del paso 2 consulta nuevamente al proveedor; un fallo conserva la fotografía anterior solo como evidencia interna y nunca la devuelve como vigente.
- La confirmación final vuelve a listar antes de persistir consentimiento, selección u operación. Si UUID + índice ya no está vigente, actualiza la fotografía, rechaza la decisión y regresa al paso 2; una solicitud ya confirmada permanece inmutable e idempotente.
