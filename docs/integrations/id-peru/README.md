...
El inicio es reintentable antes de que ID Perú retorne: una nueva solicitud finaliza como `CANCELLED` el intento cuyo `state` todavía no fue consumido, elimina su verifier protegido y genera un `state` y PKCE nuevos. Si el intento anterior ya expiró se registra como `EXPIRED`. Cuando el callback ya consumió el `state`, el backend conserva el conflicto controlado hasta que termine su procesamiento para evitar dos autenticaciones concurrentes. El retorno tardío de un intento reemplazado se rechaza y nunca puede elevar la sesión.
...
ID Perú v1 informa la cancelación voluntaria con `error=user_cancelled`; también se admite `access_denied` por compatibilidad OAuth. Ambos valores finalizan y auditan el intento como `CANCELLED`, eliminan el verifier protegido y retornan silenciosamente al paso 1, sin parámetro `identityOutcome`, cookie de presentación ni aviso de error. La solicitud conserva `PENDING_IDENTITY_VERIFICATION` y permite iniciar inmediatamente un intento nuevo. Los rechazos y fallos reales mantienen sus resultados visibles.

El frontend reconstruye el paso visible desde `nextStep` al cargar la ruta. Si el navegador restaura la página desde BFCache, vuelve a consultar la sesión para descartar el estado visual transitorio anterior. Una respuesta `IDENTITY_CONTINUITY_REQUIRED` al iniciar ID Perú también provoca una resincronización silenciosa. Si la sesión no puede resolverse, se presenta una recuperación neutral sin acción de autenticación.
