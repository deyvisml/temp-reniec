...
export function CancellationReceiptTransition({ dni, initialData }: {
  dni: string;
  initialData?: CancellationExecution;
}) {
  ...
  useEffect(() => {
    if (data) return;
    void getCurrentCancellationOutcome()
      .then(result => {
  ...
  const download = async () => {
    ...
    setError(undefined);
    try {
      const result = await requestBlob(CANCELLATION_RECEIPT_PATH, {
      ...
      anchor.href = url;
      anchor.download = `constancia-${data?.receipt?.code ?? "cancelacion"}.pdf`;
  ...
  return (
    <section className="mx-auto w-full max-w-[1040px]" aria-labelledby="receipt-title">
      <div className="px-2 sm:px-8 lg:px-14">
        <CancellationStepper currentStep={5} />
      </div>
        ...
        {data ? (
          <FlowStepContent>
            <header className="text-center">
              ...
              <p className="mt-3 text-sm leading-6 text-[#52678f] sm:text-base">El certificado digital seleccionado fue cancelado correctamente.</p>
            </header>
            <ReceiptIllustration />
            <dl className="mt-4 overflow-hidden rounded-xl bg-white ring-1 ring-[#e5e9f0] shadow-[0_5px_18px_rgba(22,46,91,0.08)]">
              ...
              <ResultRow icon="cert" label="Certificado" value={`Orden ${data.certificate.orderNumber}`} />
            ...
            <div className="mt-4 flex gap-4 rounded-xl border-l-2 border-reniec-red bg-[#fff7fa] px-4 py-3.5 text-sm leading-6 text-[#52627f] sm:px-5">
              ...
              <div>
                <p className="font-bold text-[#173568]">Se canceló únicamente el certificado digital seleccionado.</p>
            ...
            <p className="mt-5 text-center text-xs leading-5 text-[#64779d]">Esta constancia es provisional y está sujeta a validación institucional. Guárdala para tus registros.</p>
          </FlowStepContent>
        ) : null}
      </div>
    </section>
...
function MiniIcon({ kind }: { kind: string }) {
  const paths: Record<string, string> = { id: "M4 6h16v12H4zM7 10h3m-3 3h5m3-4h2m-2 4h2", cert: "M6 3h9l3 3v15H6zM9 10h6m-6 4h4", shield: "M12 3 5 6v5c0 5 3 8 7 10 4-2 7-5 7-10V6l-7-3Zm-3 8 2 2 4-4", calendar: "M5 5h14v15H5zM8 3v4m8-4v4M5 9h14", document: "M6 3h9l3 3v15H6zM9 11h6m-6 4h6" };
