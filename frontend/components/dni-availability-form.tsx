  ...
  return (
    <>
      ...
      <div className="mb-[22px] text-left">
        ...
        <div className={`flex h-[58px] items-center gap-3 rounded-lg border-[1.5px] bg-white px-[17px] transition-[border-color,box-shadow] focus-within:border-[#0755df] focus-within:shadow-[0_0_0_4px_#0755df16] max-[480px]:h-[55px] [&>svg]:w-6 [&>svg]:text-[#163e9a] ${fieldError ? "border-[#b00045]" : "border-[#7893ca]"}`}>
          ...
          <input
            ...
            aria-describedby={fieldError ? "dni-error" : undefined}
        ...
        {fieldError ? (
          <p
            id="dni-error"
            className="mt-[9px] flex items-center gap-[7px] text-xs font-bold text-[#a0003d] [&_svg]:size-4 [&_svg]:shrink-0"
          >
            <InfoIcon />
            {fieldError}
          </p>
      ...
      <span className="sr-only" aria-live="polite">
        {pending ? "Consultando la disponibilidad de credenciales verificables." : ""}
      </span>
      </form>
      {view.kind !== "form" ? (
        <AvailabilityOutcomeAlert
