...
describe("rutas del flujo ciudadano", () => {
  it("mantiene una entrada canónica y una ruta local para la autenticación", () => {
    ...
    expect(source("lib/routes.ts")).toContain('REVOCATION_FLOW_ROUTE = "/revocacion"');
    ...
    expect(activeFlowRoute("production", "development")).toBe(REVOCATION_FLOW_ROUTE);
  });
  ...
  it.each([
    ...
    expect(content).toContain("redirect(REVOCATION_FLOW_ROUTE)");
  ...
  it("mantiene home pública y solo redirige cuando existe una sesión activa", () => {
    ...
    expect(home).toContain("<PublicRevocationEntry />");
  ...
  it("renderiza el paso 1 en /autorizacion solo para el ambiente local", () => {
    ...
    const revocationPage = source("app/revocacion/page.tsx");

    expect(authorizationPage).toContain("if (!usesLocalIdentityRoute()) redirect(REVOCATION_FLOW_ROUTE)");
    expect(authorizationPage).toContain("<RevocationFlow />");
    expect(revocationPage).toContain("<RevocationFlow />");
    expect(source("app/revocacion/layout.tsx")).toContain("requireServerFlowSession");
    ...
    expect(source("app/api/session/refresh/route.ts")).toContain("revocacion_refresh");
  ...
  it("consulta el contexto protegido al recargar sin persistencia del navegador", () => {
    const flow = source("components/revocation-flow.tsx");
    expect(flow).toContain("getCurrentIdentityVerification");
    expect(flow).toContain('setView({ kind: "identity" })');
    expect(source("components/public-revocation-entry.tsx")).toContain("router.push");
